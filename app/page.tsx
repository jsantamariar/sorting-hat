...
import { OrbitControls, Environment, Text, Html, useGLTF, useProgress } from "@react-three/drei"
...
export default function SortingHatApp() {
  const [gameState, setGameState] = useState<"start" | "sorting" | "result">("start")
  const [currentQuestion, setCurrentQuestion] = useState(0)
  const [answers, setAnswers] = useState<string[]>([])
  const [selectedHouse, setSelectedHouse] = useState<keyof typeof houses | null>(null)
  ...
  const answerQuestion = (house: string) => {
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      setIsThinking(true)
      setTimeout(() => {
        const houseCounts = newAnswers.reduce(
          (acc, house) => {
  ...
  // Función para activar el hechizo Lumos
  const activateLumos = () => {
  ...
  function Loader() {
    ...
    const [loaderParticles, setLoaderParticles] = useState<Array<{
      left: string;
      top: string;
      animationDelay: string;
      animationDuration: string;
      ...
      <div className="fixed inset-0 bg-gradient-to-b from-gray-900 via-slate-900 to-black flex items-center justify-center z-50">
        <div className="text-center">
          ...
          <div className="text-sm text-white">
            ...
            {progress >= 30 && progress < 60 && "Despertando al Sombrero Seleccionador..."}
            {progress >= 60 && progress < 90 && "Cargando la Varita de Saúco..."}
  ...
  function LumosDialog() {
    return (
      ...
      >
        <div
          className="relative bg-gradient-to-br from-blue-900/95 to-cyan-900/95 border-3 border-cyan-300 rounded-2xl p-6 backdrop-blur-lg shadow-2xl border-opacity-80 max-w-sm"
          style={{
        ...
        >
          <div className="text-center relative">
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-2xl font-bold text-cyan-100 mb-2 font-serif animate-pulse drop-shadow-lg">¡LUMOS!</h2>
              <p className="text-cyan-200 text-sm drop-shadow-md leading-tight">La Varita de Saúco ilumina las tinieblas</p>
            </div>
  ...
  function GlobalClickHandler({
    ...
    useEffect(() => {
      const handleClick = (event: MouseEvent) => {
        ...
        if (camera) {
          ...
          if (hatRef.current) {
            const hatIntersects = raycaster.intersectObject(hatRef.current, true)
          ...
          if (titleRef.current && !clickedOnSpecificObject) {
            const titleIntersects = raycaster.intersectObject(titleRef.current, true)
          ...
          // Si no se clicó en ningún objeto específico, es un clic en el fondo
    ...
    }, [raycaster, pointer, camera, hatRef, titleRef, activateLumos, activateHatMagicRef, activateTitleAnimationRef])

    return null // Este componente no renderiza nada visible
  }

  return (
    <div className="w-full h-screen bg-gradient-to-b from-gray-900 via-slate-900 to-black relative overflow-hidden" style={{ cursor: "none" }}>
      ...
      <Canvas camera={{ position: [0, 2, 22], fov: 50 }} shadows style={{ cursor: "none" }}>
        ...
        <ambientLight intensity={isLumosActive ? 1.5 : 0.5} color={isLumosActive ? "#87CEEB" : "#1a1a2e"} />
        ...
        <spotLight
          ...
          color={isLumosActive ? "#87CEEB" : "#ffd700"}
          target-position={[-8, -3, 0]}
          castShadow
        />

        {/* Luz adicional para el sombrero desde abajo */}
        <pointLight position={[-8, -6, 3]} intensity={isLumosActive ? 3 : 1.5} color={isLumosActive ? "#ADD8E6" : "#ffaa00"} />

        {/* Luz de relleno para el sombrero */}
        <pointLight position={[-12, -1, 5]} intensity={isLumosActive ? 4 : 2} color={isLumosActive ? "#87CEEB" : "#ffffff"} />

        {/* Luz para la interfaz derecha */}
        <pointLight position={[5, 3, 5]} intensity={isLumosActive ? 2.5 : 1.2} color={isLumosActive ? "#ADD8E6" : "#ffffff"} />
        ...
        {isLumosActive && (
          <>
            ...
            <directionalLight position={[-5, 5, -5]} intensity={2} color="#ADD8E6" />
        ...
        <FloatingTitle titleRef={titleRef} activateTitleAnimationRef={activateTitleAnimationRef} />
        ...
        {(isThinking || selectedHouse) && <MagicalParticles selectedHouse={selectedHouse} isThinking={isThinking} />}

        {/* Partículas de niebla atmosférica */}
        <AtmosphericFog selectedHouse={selectedHouse} isThinking={isThinking} isLumosActive={isLumosActive} />
        ...
        {/* Interfaz HTML superpuesta - Posicionada a la derecha */}
        <Html position={[5, 0, -10]} center>
          <div className="w-96 text-center" style={{ cursor: "default" }}>
            {gameState === "start" && (
              <Card className="bg-black/85 border-amber-600 backdrop-blur-md shadow-2xl border-2">
                <CardContent className="p-6">
                  <h2 className="text-2xl font-bold text-amber-400 mb-4">¡Bienvenido a Hogwarts!</h2>
                  <p className="text-gray-200 mb-6 leading-relaxed">
                    El Sombrero Seleccionador determinará a qué casa perteneces. Responde honestamente a las preguntas y
                    descubre tu destino mágico.
                  </p>
                  <Button
                    onClick={startSorting}
                    className="bg-amber-600 hover:bg-amber-700 text-black font-bold px-8 py-3 shadow-lg"
                ...
                <CardContent className="p-6">
                  <div className="mb-4">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-amber-400 font-semibold">Progreso</span>
                  ...
                  <h3 className="text-xl font-bold text-amber-400 mb-4">{questions[currentQuestion].question}</h3>
                  <div className="space-y-3">
                    {questions[currentQuestion].answers.map((answer, index) => (
                      <Button
                        ...
                        variant="outline"
                        className="w-full bg-gray-800/60 border-gray-600 text-gray-200 hover:bg-gray-700/80 hover:border-amber-500 transition-all duration-300 p-4 text-left"
                        style={{ cursor: "default" }}
                      >
                        {answer.text}
                      </Button>
            ...
            {isThinking && (
              <Card className="bg-black/85 border-amber-600 backdrop-blur-md shadow-2xl border-2">
                <CardContent className="p-6">
                  <h3 className="text-xl font-bold text-amber-400 mb-4">🎩 El sombrero está deliberando...</h3>
            ...
            {gameState === "result" && selectedHouse && (
              ...
              >
                <CardContent className="p-8">
                  ...
                  <h2 className="text-4xl font-bold mb-4" style={{ color: selectedHouse === "hufflepuff" ? "#FFFFFF" : houses[selectedHouse].accent }}>
                    ¡{houses[selectedHouse].name.toUpperCase()}!
                  </h2>
                  <p className={`mb-6 text-lg leading-relaxed ${selectedHouse === "hufflepuff" ? "text-white" : "text-gray-200"}`}>
                    El Sombrero Seleccionador ha hablado. Tu lugar está en la noble casa de{" "}
                    <span className="font-bold text-2xl" style={{ color: selectedHouse === "hufflepuff" ? "#FFFFFF" : houses[selectedHouse].accent }}>
                  ...
                  <div className="space-y-3">
                    <Button
                      onClick={resetGame}
                      className="bg-amber-600 hover:bg-amber-700 text-black font-bold px-8 py-3 w-full"
                      style={{ cursor: "none" }}
                    >
                      🔄 Intentar de Nuevo
                    </Button>
      ...
      {/* Mostrar el componente LumosDialog si showLumosDialog es true */}
      {showLumosDialog && <LumosDialog />}
  ...
  useFrame((state) => {
    if (titleRef.current) {
      ...
      if (isAnimating) {
        ...
        titleRef.current.position.y = 7.5 + baseFloat + Math.sin(state.clock.elapsedTime * 8) * 0.8
        titleRef.current.position.x = Math.sin(state.clock.elapsedTime * 6) * 0.5
        titleRef.current.rotation.z = Math.sin(state.clock.elapsedTime * 10) * 0.15
        ...
        titleRef.current.position.y = 7.5 + baseFloat
        titleRef.current.position.x = Math.sin(state.clock.elapsedTime * 0.5) * 0.1
        titleRef.current.rotation.z = Math.sin(state.clock.elapsedTime * 0.6) * 0.02
...
function SortingHat({
  ...
  useFrame((state, delta) => {
    if (hatRef.current) {
      ...
      if (isSpinning) {
        ...
        hatRef.current.rotation.x = Math.sin(state.clock.elapsedTime * 10) * 0.2 // Bamboleo en X
      } else if (isThinking) {
        // Animación de "pensamiento" más dramática
        hatRef.current.rotation.z = Math.sin(state.clock.elapsedTime * 2) * 0.15
        hatRef.current.position.y = -3 + baseFloat + Math.sin(state.clock.elapsedTime * 3) * 0.3
        hatRef.current.position.x = -8 + Math.sin(state.clock.elapsedTime * 1.5) * 0.2
      } else if (gameState === "result" && selectedHouse) {
        // Animación de celebración
        hatRef.current.position.y = -3 + baseFloat + Math.sin(state.clock.elapsedTime * 4) * 0.5
        hatRef.current.rotation.z = Math.sin(state.clock.elapsedTime * 2.5) * 0.1
        hatRef.current.position.x = -8
      } else {
        // Flotación suave en reposo
        hatRef.current.position.y = -3 + baseFloat
        hatRef.current.position.x = -8 + Math.sin(state.clock.elapsedTime * 0.7) * 0.1
        hatRef.current.rotation.z = Math.sin(state.clock.elapsedTime * 0.8) * 0.05
      }
    }
        ...
        if (child instanceof THREE.Mesh && child.material) {
          ...
          materials.forEach((material) => {
            if (material instanceof THREE.MeshStandardMaterial) {
              ...
              if (isGlowing) {
                material.emissiveIntensity = 0.8 + Math.sin(state.clock.elapsedTime * 10) * 0.2
    ...
    if (scene && selectedHouse) {
      ...
      scene.traverse((child) => {
        if (child instanceof THREE.Mesh && child.material) {
          ...
          materials.forEach((material) => {
            if (material instanceof THREE.MeshStandardMaterial) {
              material.color.setHex(Number.parseInt(houses[selectedHouse].color.replace("#", "0x")))
              material.emissive.setHex(Number.parseInt(houses[selectedHouse].accent.replace("#", "0x")))
              material.emissiveIntensity = 0.15
            }
          })
        }
  ...
  return (
    <group ref={hatRef} position={[-8, -3, 0]} scale={[20, 20, 20]}>
      ...
      <mesh position={[0, -2, 0]} rotation={[-Math.PI / 2, 0, 0]} scale={[1.5, 1.5, 1]}>
        <circleGeometry args={[1, 32]} />
        <meshStandardMaterial color="#000000" transparent opacity={0.3} emissive="#000000" emissiveIntensity={0.1} />
      </mesh>
...
function HatMagicParticles() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      newParticles.push({
        position: new THREE.Vector3(Math.cos(angle) * radius, Math.random() * 0.5, Math.sin(angle) * radius),
...
function TitleMagicParticles() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      newParticles.push({
        position: new THREE.Vector3(Math.cos(angle) * radius, Math.random() * 1 - 0.5, Math.sin(angle) * radius),
        velocity: new THREE.Vector3((Math.random() - 0.5) * 0.15, Math.random() * 0.2, (Math.random() - 0.5) * 0.15),
...
function MagicalParticles({
  ...
  const particleColor = selectedHouse ? houses[selectedHouse].accent : "#ffd700"
  const particleCount = isThinking ? 25 : 35

  return (
    <group ref={particlesRef} position={[-8, -3, 0]}>
...
function AtmosphericFog({
  ...
  isThinking,
  isLumosActive,
}: { selectedHouse: keyof typeof houses | null; isThinking: boolean; isLumosActive: boolean }) {
  ...
  useFrame((state) => {
    if (fogRef.current) {
      fogRef.current.children.forEach((particle, i) => {
        ...
        const baseMovement = {
          x: Math.sin(state.clock.elapsedTime * (isLumosActive ? 1 : 0.5) + i) * (isLumosActive ? 0.006 : 0.003),
          y: Math.sin(state.clock.elapsedTime * (isLumosActive ? 0.4 : 0.2) + i) * (isLumosActive ? 0.002 : 0.001),
          z: Math.cos(state.clock.elapsedTime * (isLumosActive ? 0.6 : 0.3) + i) * (isLumosActive ? 0.004 : 0.002),
        }
  ...
  return (
    <group ref={fogRef}>
      {[...Array(particleCount)].map((_, i) => {
        ...
        const scale = isLumosActive ? 0.6 + Math.random() * 1.8 : 0.4 + Math.random() * 1.2
        return (
          <mesh
            key={i}
            position={[x, y, z]}
          ...
          >
            ...
            <meshStandardMaterial
              ...
              emissiveIntensity={isLumosActive ? 0.4 : selectedHouse ? 0.08 : 0.04}
            />
          </mesh>
        )
      })}
